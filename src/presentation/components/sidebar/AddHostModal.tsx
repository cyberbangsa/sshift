...
async function pickKeyFile(): Promise<string | null | 'use-file-input'> {
  ...
  try {
    ...
    const selected = await open({
      ...
      multiple: false,
      filters: [{ name: 'Private Key', extensions: ['pem', 'ppk', 'key', 'rsa', 'ed25519', '*'] }],
    })
...
export function AddHostModal({ isOpen, onClose, onSave, editHost }: AddHostModalProps) {
  ...
  const handleSubmit = () => {
    ...
    const host: Host = {
      ...
      vaultEntryId:
        authMethod === 'privateKey' && keySource === 'vault' && vaultEntryId
          ? vaultEntryId
          : undefined,
      privateKeyPath:
        authMethod === 'privateKey' && keySource === 'file' ? privateKeyPath.trim() : undefined,
      tags: tags
        .split(',')
        .map((t) => t.trim())
        .filter(Boolean),
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={editHost ? 'Edit Host' : 'Add Host'}>
                ...
                {(['vault', 'file'] as const).map((src) => (
                  <button
                    ...
                    style={{
                      ...
                      borderBottom:
                        keySource === src ? '2px solid #a8e8ff' : '2px solid transparent',
                      marginBottom: -1,
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {privateKeyEntries.length === 0 ? (
                    ...
                    <select
                      ...
                      style={{
                        background: '#1d2126',
                        border: `1px solid ${vaultEntryId ? '#a8e8ff55' : '#3c494e'}`,
      ...
      {activeTab === 'rules' && (
        <div className="flex flex-col gap-3">
          ...
          >
            Rules are injected into the AI agent's system prompt whenever you connect to this host.
            The AI will always follow them.
          </p>
